...
def predict():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename): #Checking file format
            ...
            class_prediction=model.predict(test_img) 
            classes_x=np.argmax(class_prediction)
            print(classes_x)
            if classes_x == 0:
              cancer = "Detected Lung adenocarcinoma"
            else:
              cancer = "Not detected Lung adenocarcinoma"
            ...
            return render_template('predict.html', user_image = file_path, cancer = cancer, fruit = fruit)
        else:
